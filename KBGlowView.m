...
- (void)finishAnimation {
    ...
    if (self.gradientLayer) {
        [self.gradientLayer removeAnimationForKey:@"fadeIn"];
        [self.gradientLayer removeAnimationForKey:@"ripple"];
        CABasicAnimation *fade = [CABasicAnimation animationWithKeyPath:@"opacity"];
        fade.fromValue = @([[self.gradientLayer.presentationLayer valueForKeyPath:@"opacity"] doubleValue]);
        fade.toValue = @0.0;
        fade.duration = 0.12;
        fade.timingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseOut];
        [self.gradientLayer addAnimation:fade forKey:@"finishFade"];
    }
    if (self.emitterLayer) {
...
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag {
    // 生命周期由触摸结束事件统一管理，避免动画刚出现就被 delegate 提前移除。
}
