...
- (void)handleGlowGesture:(UIGestureRecognizer *)gesture {
    if (!self.enabled || ![self isCurrentKeyboardEnabled]) return;
        ...
        if (self.followFinger) {
            for (UIView *subview in [w.subviews copy]) {
                if ([subview isKindOfClass:KBGlowView.class]) {
                    [(KBGlowView *)subview updateGlowPosition:p];
                }
            }
        }
...
- (void)notifySettingsChanged {
    CFPreferencesAppSynchronize((__bridge CFStringRef)kKBGlowDomain);
    CFNotificationCenterPostNotification(CFNotificationCenterGetDarwinNotifyCenter(), kKBGlowDarwinNotification, NULL, NULL, true);
}
