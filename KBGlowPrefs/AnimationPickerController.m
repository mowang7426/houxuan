...
- (void)setAnimation:(id)value specifier:(PSSpecifier *)s { if (![value boolValue]) return; NSInteger v=[[s propertyForKey:@"animationValue"] integerValue]; NSUserDefaults*d=[[NSUserDefaults alloc]initWithSuiteName:suite]; [d setInteger:v forKey:@"animationType"]; CFNotificationCenterPostNotification(CFNotificationCenterGetDarwinNotifyCenter(),note,NULL,NULL,true); [self reloadSpecifiers]; }
@end
