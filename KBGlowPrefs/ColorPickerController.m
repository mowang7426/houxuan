...
- (id)getColor:(PSSpecifier*)s { NSArray*x=[s propertyForKey:@"rgb"]; NSUserDefaults*d=[[NSUserDefaults alloc]initWithSuiteName:suite]; NSArray*c=d[@"colorRGB"]; return ([c isEqual:x]|| (c.count>=3&&fabs([c[0]doubleValue]-[x[1]doubleValue])<.001&&fabs([c[1]doubleValue]-[x[2]doubleValue])<.001&&fabs([c[2]doubleValue]-[x[3]doubleValue])<.001))?@YES:@NO; }
- (void)setColor:(id)value specifier:(PSSpecifier*)s { if(![value boolValue])return; NSArray*x=[s propertyForKey:@"rgb"]; NSUserDefaults*d=[[NSUserDefaults alloc]initWithSuiteName:suite]; d[@"colorRGB"]=@[x[1],x[2],x[3]]; [d synchronize]; CFNotificationCenterPostNotification(CFNotificationCenterGetDarwinNotifyCenter(),note,NULL,NULL,true); [self reloadSpecifiers]; }
- (id)getValue:(PSSpecifier*)s { NSUserDefaults*d=[[NSUserDefaults alloc]initWithSuiteName:suite]; id v=d[[s propertyForKey:@"key"]]; return v?:@0.0; }
- (void)setValue:(id)v specifier:(PSSpecifier*)s { NSUserDefaults*d=[[NSUserDefaults alloc]initWithSuiteName:suite]; d[[s propertyForKey:@"key"]]=v; [d synchronize]; }
- (void)applyCustom:(PSSpecifier*)s { NSUserDefaults*d=[[NSUserDefaults alloc]initWithSuiteName:suite]; CGFloat r=[d doubleForKey:@"customR"],g=[d doubleForKey:@"customG"],b=[d doubleForKey:@"customB"]; d[@"colorRGB"]=@[@(r),@(g),@(b)]; [d synchronize]; CFNotificationCenterPostNotification(CFNotificationCenterGetDarwinNotifyCenter(),note,NULL,NULL,true); }
@end
